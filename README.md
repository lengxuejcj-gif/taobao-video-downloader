...
网页工具，用于粘贴淘宝/天猫商品链接、淘宝真实 MP4 视频链接或抖音分享文案，按产品名称保存视频。
...
1. 粘贴淘宝/天猫商品链接、淘宝真实视频 MP4 链接，或在抖音复制完整分享文案。
2. 粘贴到网页里的“视频链接 / 分享文案”。
3. 填产品名称。
...
淘宝/天猫商品页会先尝试解析页面里公开暴露的视频地址和标题。如果淘宝返回登录或验证页面，服务器无法读取商品视频，需要在商品页复制真实 MP4 视频链接后再下载。

淘宝视频链接如果带 `auth_key`，它可能会过期，建议复制后尽快下载。

抖音会优先解析页面里能拿到的最高播放源；如果普通页面被风控，Docker 线上版本会用 Chromium + yt-dlp 生成临时浏览器 Cookie 后兜底解析。部分私密、下架、地区限制、验证码或强登录的视频仍可能无法解析，需要复制真实视频链接。
